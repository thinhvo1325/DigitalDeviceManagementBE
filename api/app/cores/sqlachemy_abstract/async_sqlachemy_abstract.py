...
class SqlAchemyAbstract(SqlAchemyContracts):
    ...
    @abstractmethod
    async def search_with_raw_query(self, query, order: dict = None, sort_schema: dict = None, is_get_first: bool = False, with_join: bool = False):
        ...
        if sort_schema is not None:
            if sort_schema['order'] == 'desc':
                query = query.order_by(desc(getattr(self._model, sort_schema['sort_by'])))
            if sort_schema['order'] == 'asc':
                query = query.order_by(asc(getattr(self._model, sort_schema['sort_by'])))
